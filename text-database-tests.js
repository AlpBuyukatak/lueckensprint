...
const required={A1:50,A2:70,B1:120,B2:140,C1:70},levels=Object.keys(required),all=[];
const tokens=text=>new Set((text.toLowerCase().match(/[a-zäöüß]+/gu)||[]).filter(word=>word.length>2));
const similarity=(left,right)=>{const a=tokens(left),b=tokens(right),shared=[...a].filter(word=>b.has(word)).length;return shared/(a.size+b.size-shared||1)};
const report={levels:{},topics:{},genres:{},duplicates:[]};
const requiredFields=['genre','wordCount','sentenceCount','difficultyScore','estimatedCompletionTime','grammarTags','vocabularyTags','register','sourceType','createdVersion','qualityReviewStatus','averageSentenceLength','lexicalDiversityEstimate','subordinateClauseCount','compoundNounCount','nominalizationEstimate','eligibleCtestWords'];
for(const level of levels){
  ...
  for(const row of rows){
    ...
    assert.equal(row.sourceType,'original');assert.ok(!/<\/?[a-z][^>]*>/i.test(row.text),`${row.id}: HTML found`);assert.ok(!/https?:\/\/|www\.|lorem ipsum|\ufffd/i.test(row.text),`${row.id}: invalid source text`);assert.ok(/[.!?]$/.test(row.text.trim()),`${row.id}: unfinished text`);assert.ok(ctest(row.text,{gaps:20}).gapCount>=20,`${row.id}: too few C-Test gaps`);
...
fs.writeFileSync(path.join(__dirname,'data-quality-report.json'),JSON.stringify(report,null,2),'utf8');fs.writeFileSync(path.join(__dirname,'ctest-duplicate-report.json'),JSON.stringify({generatedAt:new Date().toISOString(),method:'Normalized token-set Jaccard similarity within each CEFR level; pairs above 0.93 are warnings.',duplicateWarnings:report.duplicates,warningCount:report.duplicates.length,finalTextCount:all.length},null,2),'utf8');
console.log(`Text database validation passed: ${all.length} original texts`);console.log(JSON.stringify(report.levels));
