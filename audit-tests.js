...
assert.ok(app.includes('if(location.hash===route){render();return}'),'Navigating to the current route must re-render it; result-page training buttons otherwise only scroll to the top.');
assert.ok(app.includes("ROUTES.miniExam='#/mini-pruefung'")&&app.includes('Mit ${active.text.level} · ${active.text.topic} weitertrainieren'),'Mini exam route and same-level result continuation are missing.');
...
assert.ok(sw.includes('lueckensprint-v2.5.0'),'Service worker cache version was not bumped for the continuation and mini-exam feature.');
